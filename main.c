...
int main(int argc, char* argv[]) {
	...
	for (i=0; i<nbTriCenters; i++){
		triCenters[i] = create_tri_center(&trucks[i*NB_TRUCKS_PER_TRI_CENTER], NB_TRUCKS_PER_TRI_CENTER, PERIOD, rand()%MAX_X, rand()%MAX_Y, &triPoints[i*TRI_POINTS_PER_TRI_CENTER], TRI_POINTS_PER_TRI_CENTER);
